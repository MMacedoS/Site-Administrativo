...
<div class="col-md-12 my-3">
	<a href="#" onclick="inserir()" type="button" class="btn btn-dark btn-sm">Nova Célula</a>
</div>

<div class="tabela bg-light">
	<?php 

	$res = $this->getCelulas($id_igreja);
	$total_reg = count($res);
	if($total_reg > 0){
		...
		<table id="example" class="table table-striped table-light table-hover my-4 my-4" style="width:100%">
			...
			<tbody>
				...
				for($i=0; $i < $total_reg; $i++){
					...
					$res2 = $this->getCelulasMembros($id_igreja,$id);
					...
					$res_con = $this->getMembrosId($coordenador);
					if(count($res_con) > 0){
...
<div class="modal fade" id="modalForm" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	...
	function listarMembrosCB(celula, igreja){
		...
		$.ajax({
        url: "<?=ROTA_IGREJA?>Home/celulas/listar-membros.php",
        method: 'POST',
        data: {celula, igreja},
        dataType: "text",
	...
	function listarMembrosAdd(celula, igreja){
		...
		$.ajax({
        url: "<?=ROTA_IGREJA?>Home/celulas/listar-membros-add.php",
        method: 'POST',
        data: {celula, igreja},
        dataType: "text",
